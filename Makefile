...
ifneq (,$(findstring "$(PLATFORM)", "linux" "gnu_kfreebsd" "kfreebsd-gnu"))
  ...
  OPTIMIZEVM = -O3 -g -funroll-loops -fno-omit-frame-pointer
  OPTIMIZE = $(OPTIMIZEVM) -ffast-math

  ifeq ($(ARCH),x86_64)
	OPTIMIZEVM = -O3 -g -fno-omit-frame-pointer -funroll-loops \
  ...
  ifeq ($(ARCH),x86)
	OPTIMIZEVM = -O3 -g -march=i586 -fno-omit-frame-pointer \
...
ifeq ($(PLATFORM),mingw32)
  ...
  ifeq ($(ARCH),x86_64)
	OPTIMIZEVM = -O3 -g -fno-omit-frame-pointer \
...
ifndef BASEGAME_CXXFLAGS
  BASEGAME_CXXFLAGS=$(BASEGAME_CFLAGS)
endif

ifndef MISSIONPACK_CXXFLAGS
  MISSIONPACK_CXXFLAGS=$(MISSIONPACK_CFLAGS)
endif

ifndef SHLIBCXXFLAGS
...
$(B)/$(BASEGAME)/cgame$(SHLIBNAME): $(Q3CGOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CXX) $(CXXFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3CGOBJ)
...
$(B)/$(MISSIONPACK)/cgame$(SHLIBNAME): $(MPCGOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CXX) $(CXXFLAGS) $(SHLIBLDFLAGS) -o $@ $(MPCGOBJ)
...
Q3GOBJ = $(Q3GOBJ_) $(B)/$(BASEGAME)/game/g_syscalls.o $(B)/$(BASEGAME)/game/DoubleBuffering.o
Q3GVMOBJ = $(Q3GOBJ_:%.o=%.asm)

$(B)/$(BASEGAME)/qagame$(SHLIBNAME): $(Q3GOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CXX) $(CXXFLAGS) $(SHLIBLDFLAGS) -o $@ $(Q3GOBJ)
...
MPGOBJ = $(MPGOBJ_) $(B)/$(MISSIONPACK)/game/g_syscalls.o $(B)/$(BASEGAME)/game/DoubleBuffering.o
MPGVMOBJ = $(MPGOBJ_:%.o=%.asm)

$(B)/$(MISSIONPACK)/qagame$(SHLIBNAME): $(MPGOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CXX) $(CXXFLAGS) $(SHLIBLDFLAGS) -o $@ $(MPGOBJ)
